...
double DonorCell::computeDuvDx (int i, int j) const {
  return (1 / (4 * meshWidth_[1])) * (((v(i,j) + v(i+1,j)) * (u(i,j+1) + u(i,j))) - (v(i,j) + v(i-1,j)) * (u(i-1,j) + u(i-1,j+1))) +
  (alpha_ / (4 * meshWidth_[1])) * (abs(u(i,j+1) + u(i,j)) * (v(i+1,j) - v(i,j)) - abs(u(i-1,j) + u(i-1,j+1)) * (v(i,j) - v(i-1,j)));
}
...
double DonorCell::computeDv2Dy (int i, int j) const {
  return (1 / (4 * meshWidth_[0])) * (pow(v(i,j+1) + v(i,j), 2) - pow(v(i,j) + v(i,j-1), 2)) + 
  alpha_ * (1 / (4 * meshWidth_[0])) * (abs(v(i,j+1) + v(i,j)) * (v(i,j) - v(i,j+1)) - abs(v(i,j) + v(i,j-1)) * (v(i,j-1) - v(i,j)));
}
